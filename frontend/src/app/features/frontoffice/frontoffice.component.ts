      ...
      <div *ngIf="activeSection === 'formations'">
        <div class="filters glass-panel mb-4 d-flex justify-between align-center">
          <div class="form-group mb-0 search-bar">
            <input type="text" class="form-control" placeholder="Search by title..." [(ngModel)]="searchQuery" (keyup.enter)="onSearch()">
          </div>
          ...
          <button class="btn btn-primary" (click)="loadFormations()">Reset</button>
        </div>
...
export class FrontofficeComponent implements OnInit {
  ...
  loadFormations() {
    ...
    this.formationService.getActive(0, 100).subscribe({
      next: (data) => this.formations = data.content,
  ...
  onSearch() {
    if (this.searchQuery.trim()) {
      this.formationService.searchByTitle(this.searchQuery).subscribe({
  ...
  onCategoryChange() {
    if (this.selectedCategory) {
      this.formationService.filterByCategory(this.selectedCategory).subscribe({
        next: (data) => this.formations = data,
        error: (err) => console.error(err)
      });
    } else {
